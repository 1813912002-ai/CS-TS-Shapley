...
from ts_shapley import ts_shapley
from cs_ts_shapley import cs_ts_shapley
...
class DataValuation(object):
    ...
    def estimate(self, clf=None, method='tmc-shapley', params=None):
        '''
        clf - a classifier instance (Logistic regression, by default)
        method - the data valuation method (TMC-Shapley, by default)
        ...
        if method == 'tmc-shapley':
        ...
        elif method == 'ts-shapley':
            # TS Shapley (TrustScore + TMC Shapley)
            # 使用与tmc-shapley相同的参数
            n_iter = self.params.get('ts_iter', self.params.get('tmc_iter', 500))
            tmc_thresh = self.params.get('tmc_thresh', 0.001)
            # 调用ts_shapley函数，T参数对应迭代次数
            vals = ts_shapley(self.trnX, self.trnY, self.devX, self.devY,
                             self.clf, T=n_iter, epsilon=tmc_thresh)
            for idx in range(len(vals)):
                self.values[idx] = vals[idx]
            ...
            for idx in range(len(vals)):
                self.values[idx] = vals[idx]
        elif method == 'cs-ts-shapley':
            # CS-TS Shapley (Class-conditional + TrustScore Shapley)
            # 使用与cs-shapley相同的迭代参数
            n_iter = self.params.get('cs_iter', 500)
            cs_thresh = self.params.get('cs_thresh', 0.001)
            labels = list(set(self.trnY))
            for label in labels:
                vals, orig_indices = cs_ts_shapley(self.trnX, self.trnY, self.devX,
                                                self.devY, label,
                                                self.clf, T=n_iter, epsilon=cs_thresh)
                for (idx, val) in zip(list(orig_indices), list(vals)):
                    self.values[idx] = val
        else:
